...
'''
# BFS to determine levels of graph
levels = [0] * k
q = deque()
q.append((0,0)) # (tripId, level)
while q:
	curTrip, curLevel = q.popleft()
	levels[curLevel] += 1
	for nextTrip in trips[curTrip][4]:
		q.append((nextTrip, curLevel + 1))
'''
...
while s:
	d += 1
	q.append(heapq.popleft(s))
	while q:
		currTrip = trips[]

		for next in currTrips[4]
			s.remove(i)
			q.append(i)
...
drivers: list[tuple[int, int]] = [] # (ending_city_index, available_at_time)[]
for t2, u, v, t1, next in trips:
	moveDistance: list[tuple[int, int]] = [] # (alignmentDriveTime, driverIndex)[]
