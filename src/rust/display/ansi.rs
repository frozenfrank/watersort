...
fn ansi_fore_s(rgb: RGB, s: &mut String) -> Result<(), std::fmt::Error> {
    let RGB (r, g, b) = rgb;
    write!(s, "{ANSI_ESCAPE}38;2;{r};{g};{b}m")
}

fn ansi_back_s(rgb: RGB, s: &mut String) -> Result<(), std::fmt::Error> {
    let RGB (r, g, b) = rgb;
